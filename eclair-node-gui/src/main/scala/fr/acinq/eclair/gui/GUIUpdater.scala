...
class GUIUpdater(mainController: MainController) extends Actor with ActorLogging {
  ...
  def updateBalance(channelPaneController: ChannelPaneController, commitments: Commitments) = {
    ...
    channelPaneController.capacity.setText(s"${CoinFormat.MILLI_ATB_FORMAT.format(millisatoshi2btc(MilliSatoshi(spec.totalFunds)).amount)}")
    channelPaneController.amountUs.setText(s"${CoinFormat.MILLI_ATB_FORMAT.format(millisatoshi2btc(MilliSatoshi(spec.toLocalMsat)).amount)}")
